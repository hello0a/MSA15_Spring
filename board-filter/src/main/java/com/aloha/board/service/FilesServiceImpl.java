...
public class FilesServiceImpl implements FilesService {
    ...
    public int upload(List<MultipartFile> files, ParentTable parentTable, Integer parentNo) throws Exception {
        ...
        // 다음 정렬 순서 - sortOrder : 0 또는 기존 파일 마지막 순서 +1
        int sortOrder = filesMapper.nextSortOrderByParent(fileSort);
        // 파일 여러개 업로드했을 때
        // : 첫번째 파일 sortOrder=0
        // : 두번째 파일 sortOrder=1 ,,,
        // -> 순서 저장하기 위한 번호
    ...
    public int deleteByParent(Files files) throws Exception {
        // 파일 목록 조회
        List<Files> filesList = listByParent(files);
        // 파일 삭제
        ...
        // DB 파일 삭제
        int result = filesMapper.deleteByParent(files);
        return result;
    }
    ...
    public boolean updateSortOrder(List<Map<String, Object>> sortOrderList) throws Exception {
        ...
        for (Map<String,Object> map : sortOrderList) {
            ...
            result += filesMapper.updateById(file);
        }
    ...
    public boolean updateFileSortOrder(List<Files> fileList) throws Exception {
        ...
        for (Files files : fileList) {
            ...
            Files updatFiles = new Files();
            updatFiles.setId(id);
            // index 0 이면 메인 파일
            if ( index == 0 ) {
                updatFiles.setIsMain(true);
            } else {
                updatFiles.setIsMain(false);
            }
            updatFiles.setSortOrder(index++);
            result += filesMapper.updateById(updatFiles);
        }
