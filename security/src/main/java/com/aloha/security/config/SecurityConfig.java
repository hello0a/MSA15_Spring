...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        ...
        http.formLogin( login -> login.loginPage("/login") // 커스텀 로그인 페이지 경로
                                ...
                                .successHandler(loginSuccessHandler)    // 로그인 성공 행들로 설정
                                .failureHandler(loginFailureHandler));  // 로그인 실패 핸들러 설정
        // 인가 설정되어 있어야 폼 로그인 실행 가능!

        // 로그아웃 설정
        http.logout(logout -> logout
            .logoutUrl("/logout")   // 로그아웃 요청 경로 -> 바꾸고 싶다면 경로 바꾸기
            ...
            // successurl 과 successhandler 둘 중 하나만!
        );
        ...
        http.rememberMe(me -> me 
                        ...
                        .tokenValiditySeconds(60 * 60 * 24 * 7)    // 자동 로그인 토큰 유효 기간 -> 일주일 내 계속 들어오면 계속 연장!
        );
        // filter 가 토큰 있다면 db 보내고, persistent_logins 에서 확인 후 user에 있다면 security에 등록!
        return http.build();
    }
    /**
     * 암호화 방식 빈 등록
    ...
    /**
     * JDBC 인증 방식으로 사용자정보 서비스 빈 등록
     * @return
     */
    // @Bean
    // public UserDetailsService userDetailsService() {
    //     // JdbcUserDetailsManager 통해서 어떤 DB에 접속할건지 알려주기!
    ...
    */
    ...
    public AuthenticationManager authenticationManager( 
        // AuthenticationConfiguration 인증 설정 매개변수 가져와서 등록??
